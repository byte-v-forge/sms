...
- 激活热状态：Redis，key 前缀 `sms:activation`
...
- 使用方负责选择具体报价和上游 provider；SMS 服务不承载路由策略、fallback、价格上限或 route profile。
...
- poll worker 查询 provider 状态；收到验证码或状态变化后发布公共 SMS 事件。
...
- `PLATFORM_REDIS_URL`：激活热状态 Redis
- `SMS_PROVIDER_HTTP_PROXY`：可选 provider HTTP 代理
