...
func (s *SMSCodeSecretStore) ResolveSecret(ctx context.Context, ref *commonv1.SecretRef) (string, error) {
	if s == nil || s.store == nil {
		return "", errors.New("sms code secret store is not configured")
	}
	if err := secretref.Validate(ref); err != nil {
		return "", err
	}
	if ref.GetProvider() != "sms" || ref.GetPurpose() != "sms_otp" {
		return "", errors.New("sms secret ref scope mismatch")
	}
	...
	if !ok {
		return "", errors.New("sms code secret ref is not resolvable")
	}
	return value, nil
}
